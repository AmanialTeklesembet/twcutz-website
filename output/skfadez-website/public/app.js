const translations = {
  no: {
    ...
    contactCopy: "Send en melding, ring eller send e-post for rask kontakt.",
    ...
    minutes: "min",
    homeGallery: "Se klipp, fades og detaljer.",
    homeServices: "Klipp, skjegg og line-up.",
  ...
  en: {
    ...
    contactCopy: "Send a message, call or email for quick contact.",
    ...
    minutes: "min",
    homeGallery: "See cuts, fades and details.",
    homeServices: "Haircut, beard and line-up.",
...
function renderAdminSummary() {
  ...
  const slots = state.adminData.availabilitySlots || [];
  const messages = state.adminData.messages || [];
  const upcomingSlots = slots.filter(slot => slot.active && slot.status === "ledig").length;
  const newBookings = bookings.filter(booking => booking.status === "ny").length;
  const unreadMessages = messages.filter(message => message.status === "ny").length;
  summary.innerHTML = `
    <article class="summary-card"><span>${newBookings}</span><small>Nye bookinger</small></article>
    <article class="summary-card"><span>${upcomingSlots}</span><small>Ledige tider</small></article>
    <article class="summary-card"><span>${state.adminData.services.length}</span><small>Tjenester</small></article>
    <article class="summary-card"><span>${unreadMessages}</span><small>Nye meldinger</small></article>
  `;
}
  ...
  pane.innerHTML = state.adminData.bookings.map(booking => `
    <article class="admin-card">
      <div class="admin-card-header">
        ...
        <select class="admin-select" data-booking-status="${escapeHtml(booking.id)}">
          ${["ny", "bekreftet", "fullført", "kansellert"].map(status => `<option ${booking.status === status ? "selected" : ""}>${status}</option>`).join("")}
        </select>
      </div>
      <span class="${statusClass(booking.status)}">${escapeHtml(booking.status)}</span>
        ...
        <article class="admin-card">
          <div class="admin-card-header">
            <div>
              ...
              <small>${slot.active ? "aktiv" : "inaktiv"}${slot.note ? ` - ${escapeHtml(slot.note)}` : ""}</small>
            </div>
          ...
          <span class="${statusClass(slot.status)}">${escapeHtml(slot.status)}</span>
        </article>
  ...
  pane.innerHTML = state.adminData.messages.map(message => `
    <article class="admin-card">
      <div class="admin-card-header">
        ...
        <select class="admin-select" data-message-status="${escapeHtml(message.id)}">
